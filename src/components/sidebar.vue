<template>
  ...
  <ADrawer
    ...
    :closable="false"
    placement="left"
    :bodyStyle="{ padding: 0, background: '#0f0e0e' }"
    :width="280"
  >
        ...
        <ADropdown trigger="click">
          ...
          >
            <div class="flex items-center gap-[10px]">
              <p class="text-[14px] mb-0 font-semibold text-[#cccccc]">Admin</p>
            </div>
            <Icon icon="ion-ios-arrow-down" />
          </div>
          <template #overlay>
            <MbDropdownContainer>
              <div class="text-[#cccccc] flex px-[12px] py-[10px] items-center justify-between">
                Admin
              </div>
              <MbHr />
              <MbLine @click="logout" class="justify-between" clickable>
                <p class="text-[14px] text-[#cccccc]">Выход</p>
    ...
    <div class="px-[20px]">
      <router-link v-for="link in links" :key="link.name" :to="link.href">
        <MbLine class="text-[#cccccc]" clickable>
          <Icon :icon="link.icon" />
          <p class="text-[12px] text-[#cccccc] font-semibold">{{ link.name }}</p>
        </MbLine>
      </router-link>
    </div>
  </ADrawer>
...
const links = computed(() => {
  return [
    { name: 'Partner', icon: 'fluent:document-bullet-list-16-regular', href: '/partner' },
    { name: 'Products', icon: 'fluent:document-bullet-list-16-regular', href: '/products' },
  ];
});
